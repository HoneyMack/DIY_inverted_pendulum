...
//      PowerI = 0.0;
    ...
    else if(abs(sens.grole- BAL_DEG)<5){
      if(abs(PowerD - PowerP) > abs(PowerD + PowerP))
        value = PowerP * 36.0 + PowerI  *5.45 + PowerD * 1000.0;
      else
        value = PowerP * 48.0 + PowerI * 5.45 + PowerD * 5000.0;
    }
    else{
      if(abs(PowerD - PowerP) > abs(PowerD + PowerP))
...
//        Serial.println("HIGH");
  ...
  if(digitalRead(RESET_SWITCH) == HIGH&&switch_down_count<0){
    Serial.println("HIGH");
    sens.grole =0.0;
    switch_down_count = 700;
      }
}
